...
stream_videos() {
    ...
    find "${VIDEO_DIR}" -type f \( -name '*.mp4' -or -name '*.mkv' \) | while read -r file; do
        ...
        local FFMPEG_CMD="ffmpeg -re -nostdin -i \"$file\" -map 0 -flags +global_header -c:v libx264 -c:a aac -preset ultrafast -minrate 3000k -maxrate 6000k -bufsize 3000k -g 30 -b:a 128k -ar 44100 -vf \"scale=1920:1080\" -r 30"
